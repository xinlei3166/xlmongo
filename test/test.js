...
describe('delete', () => {
  ...
  test('deleteOne', () => {
    return m.deleteOne('pic', {_id: '5f0d86399086556e10c15aca'}).then(res => {
      if (res) {
...
describe('update', () => {
  ...
  test('updateOne', () => {
    return m.updateOne('pic', {_id: '5f0eb289c6d7f27b8a498ed9', name: '测试'}, {$set:{name: '测试11111'}}).then(res => {
      if (res) {
        expect(res).toHaveProperty('n')
        expect(res.n).toBeGreaterThan(0)
      }
  ...
  test('updateMany', () => {
    return m.updateMany('pic', {name: '测试1'}, {$set:{name: '测试11111'}}).then(res => {
      if (res) {
        expect(res).toHaveProperty('n')
        expect(res.n).toBeGreaterThan(0)
      }
...
describe('find', () => {
  const obj = {_id: expect.anything(String), name: expect.anything(String), url: expect.anything(String)}
  test('findOne', () => {
    return m.findOne('pic', {_id: '5f0eb289c6d7f27b8a498ed9', name: '测试11111'}).then(res => {
      if (res) {
